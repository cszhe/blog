...
(function () {
  ...
  /*
   * Word-by-word streaming reveal — ChatGPT-style typing for blog posts.
   *
   * Text nodes are split into per-word <span class="tw-word"> elements.
   * Inline elements (a, strong, em, code) keep their HTML structure.
   * Pre and heading blocks are left untouched.
   *
   * Performance optimizations for iPhone/Safari:
   *   - Batch reveal: 3 words per tick at 40ms, not 1 at a time
   *   - content-visibility: hidden on unrevealed spans (skips layout)
   *   - CSS animation instead of transition (GPU-composited, no concurrent
   *     transitions fighting for the compositor thread)
   *   - Skip is O(words), but each word span is just a class toggle
   */
